...
export default {
  ...
  process_inline_comment(line) {
    let hash_idx = line.indexOf('#');
    return (hash_idx > -1)? line.slice(0,hash_idx):line;
  },

  process_line(line, dir) {
    let pack_ar = [];
    line = this.process_inline_comment(line);
    let line_ar = line.trim().split(/[ ]+/);
